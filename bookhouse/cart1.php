...
echo $users;
//$qty=$_POST['qty'];
$queryy="select bookname from books where bookno=$bookno";
$rs1=mysql_query($queryy);
$row = mysql_fetch_row($rs1);
$queryt="insert into transactionmaster(transactionno,transactiondate,transactionusername,bookname)values('$transactionno',NOW(),'".$_SESSION['txtusername']."','".$row[0]."')"; 
...
function cost($bookno)
{
 ...
 //$totalCost2=0;
    $sql = "SELECT *
            FROM cart
            INNER JOIN books
            ON cart.item = books.bookno where books.bookno=$bookno";
